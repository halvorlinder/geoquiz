import linesData from '../../data/border-lines.json'
...
export type BorderQuestion = Readonly<{ id: string; codes: readonly [string, string]; path: readonly BorderPosition[]; source: string; difficulty: BorderDifficulty; knownCode?: string; answerCode?: string }>

type EncodedLine = readonly [string, string, 0 | 1, readonly number[]]
...
export function borderLineContainedInShape(question: BorderQuestion, code: string): boolean {
  ...
  return question.path.length>=2 && question.path.slice(1).every((point,index)=>segments.has(segmentKey(question.path[index],point)))
}
...
export function borderQuestions(difficulty: BorderDifficulty, continent: BorderContinent = 'All', random: () => number = Math.random): BorderQuestion[] {
  ...
  for (const line of data.lines.filter((line) => eligible(line, continent, difficulty))) {
    const [left, right, source, encoded] = line; const codes = [left, right] as const; const path = decode(encoded)
    if (difficulty === 'hard') { questions.push(Object.freeze({ id: `border-hard:${left}-${right}`, codes, path, source: source === 0 ? 'current-shapes' : 'overture', difficulty })); continue }
    ...
    questions.push(Object.freeze({ id: `border-easy:${left}-${right}:${knownCode}`, codes, path, source: source === 0 ? 'current-shapes' : 'overture', difficulty, knownCode, answerCode }))
  }
  return questions
}
