...
import { BORDER_DATA_VERSION, borderContinents, borderEntity, borderQuestions, borderSetupNote, type BorderContinent, type BorderDifficulty, type BorderQuestion } from './borderCountries'

type Mode = 'practice' | 'timed'
type Config = Readonly<{ mode: Mode; difficulty: BorderDifficulty; continent: BorderContinent }>
type State = Readonly<{ value: string; found: readonly string[]; revealed: boolean; complete: boolean }>
const blank = (): State => ({ value: '', found: [], revealed: false, complete: false })
...
/** Direct Chrome-only harness; Vite replaces DEV with false and removes this branch from production. */
...
export function BorderCountriesQuiz({ questionFactory = borderQuestions, random }: BorderCountriesQuizProps) {
  ...
  function submit(value: string, timedMode=false) {
    ...
    const match=matchNeighbourAnswer(value,required,state.found,studyEntities,{timed:timedMode})
    if(match.status==='correct-new') { const found=[...state.found,match.entity.code]; const finished=found.length===required.length; update(old=>({...old,value:'',found,complete:finished})); setFeedback(finished ? timedMode ? 'Correct. Next border.' : 'Correct. Continue when you are ready.' : `${match.entity.name} accepted. One country remains.`); if(finished&&timedMode){actionLock.current=true;done('correct')} window.setTimeout(()=>inputRef.current?.focus(),0); return }
  ...
  function reveal(event?: MouseEvent<HTMLButtonElement>){ if(event&&event.detail>1||!target||(active.mode==='timed'&&actionLock.current))return; if(active.mode==='timed') actionLock.current=true; update(old=>({...old,revealed:true,complete:true,value:''})); setFeedback('Answer revealed. Continue when you are ready.'); if(active.mode==='timed'){ setAcknowledgement(target); done('reveal') } }
  function next(){ if(!target)return; if(active.mode==='timed') return; actionLock.current=false; setPractice(old=>advanceQuiz(old,!state.revealed)); setPracticeState(blank()); setPresentationEpoch(value=>value+1); setFeedback(active.difficulty==='hard'?'Name both countries sharing this border.':'Name the country on the other side of the highlighted border.') }
  ...
  return <main className="app-shell quiz-active-shell border-quiz-shell"><header className="masthead"><div><p className="eyebrow">Border quiz</p><h1>Country borders</h1></div><div className="timed-summary">{active.mode==='timed'&&timed&&<p className="timer">Time {clock(elapsedTimedSessionMs(timed,now))}</p>}<p className="progress"><span>{active.mode==='timed'&&timed?timed.pendingQuestionIds.length:Math.max(0,practice.deck.length-practice.index)} remaining</span><strong>{active.difficulty==='hard'?'Hard':'Easy'} · {active.continent}</strong></p></div></header>
    ...
    {target&&<section className={`country-capital-card border-question-card${state.revealed?' border-revealed':''}${state.complete&&!state.revealed?' border-correct':''}`} aria-labelledby="border-question-title">{target.difficulty==='easy'?<EasyBorderContextMap key={target.id} question={target} answerStatus={active.mode==='practice'?(state.revealed?'revealed':state.complete?'correct':undefined):undefined} recenterEpoch={presentationEpoch+(state.complete||state.revealed?1:0)}/>:<BorderLineImage question={target}/>}<p className="border-source"><a href={`${import.meta.env.BASE_URL}border-data-sources.html`}>Border data sources</a></p><h2 id="border-question-title">{heading}</h2><p className="country-capital-instruction">{target.difficulty==='hard'?'Type each country in any order.':'Type the other country sharing the marked land border.'}</p>{state.found.length>0&&<ul className="border-answer-chips" aria-label="Answered countries">{state.found.map(code=><li key={code}>{borderEntity(code)?.name}</li>)}</ul>}{state.revealed&&<p className="border-reveal-answer">{target.codes.map(code=>borderEntity(code)?.name).join(' and ')}</p>}<form onSubmit={event=>{event.preventDefault();submit(state.value,active.mode==='timed')}}><label htmlFor="border-answer">{target.difficulty==='hard'?'Country':'Other country'}</label><input ref={inputRef} id="border-answer" value={state.value} disabled={state.complete||state.revealed} autoComplete="off" onCompositionStart={()=>{composing.current=true}} onCompositionEnd={(event)=>{composing.current=false;if(active.mode==='timed')submit(event.currentTarget.value,true)}} onChange={(event:ChangeEvent<HTMLInputElement>)=>{update(old=>({...old,value:event.target.value}));if(active.mode==='timed'&&!composing.current)submit(event.target.value,true)}}/><div className="country-capital-actions">{active.mode==='practice'&&!state.complete&&!state.revealed&&<button className="primary-button" type="submit">Check answer</button>}{!state.complete&&!state.revealed&&<button className="text-button" type="button" onKeyDown={rejectRepeat} onClick={reveal}>Reveal answer{target.difficulty==='hard'?'s':''}</button>}{active.mode==='timed'&&!state.complete&&!state.revealed&&<button className="text-button" type="button" onKeyDown={rejectRepeat} onClick={(event)=>{if(event.detail>1||!timed||actionLock.current)return;actionLock.current=true;const sole=timed.pendingQuestionIds.length===1;setTimed(transitionTimedSession(timed,'skip',performance.now()));setFeedback(sole?'Skipped. This is the only pending border.':'Skipped. It will return after the other pending borders.');window.setTimeout(()=>inputRef.current?.focus(),0)}}>Skip</button>}{(state.complete||state.revealed)&&active.mode==='practice'&&<button ref={nextRef} className="primary-button" type="button" onKeyDown={rejectRepeat} onClick={next}>Next border</button>}</div></form><p className={`feedback ${state.revealed?'revealed':state.complete?'correct':''}`} role="status" aria-live="polite">{feedback}</p></section>}</main>
}
