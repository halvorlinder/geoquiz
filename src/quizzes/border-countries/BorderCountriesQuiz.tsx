...
export function BorderCountriesQuiz({ questionFactory = borderQuestions, random }: BorderCountriesQuizProps) {
  ...
  const inputRef=useRef<HTMLInputElement>(null); const nextRef=useRef<HTMLButtonElement>(null); const restartRef=useRef<HTMLButtonElement>(null); const acknowledgementRef=useRef<HTMLElement>(null); const composing=useRef(false); const recorded=useRef<TimedSession|null>(null); const actionLock=useRef(false); const pausedRef=useRef(false)
  ...
  function submit(value: string, timedMode=false) {
    if(!target||state.complete||state.revealed||(timedMode&&pausedRef.current)||(timedMode&&actionLock.current))return
  ...
  function pauseTimedRun(){ if(!timed||!target||pausedRef.current)return; const now=performance.now(); setNow(now); pausedRef.current=true; setTimed(session=>session?pauseTimedSession(session,now):session) }
  function resumeTimedRun(){ if(!pausedRef.current)return; const now=performance.now(); setNow(now); pausedRef.current=false; setTimed(session=>session?resumeTimedSession(session,now):session) }
